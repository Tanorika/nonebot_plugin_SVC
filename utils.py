...
from nonebot_plugin_SVC.hubert import hubert_model
...
def get_hubert_model():
  vec_path = current_path+"\\hubert\\checkpoint_best_legacy_500.pt"
...
def get_hparams(init=True):
  ...
  parser.add_argument('-c', '--config', type=str, default=current_path+"/configs/base.json",
                      help='JSON file for configuration')
  parser.add_argument('-m', '--model', type=str, required=True,
                      help='Model name')

  args = parser.parse_args()
  model_dir = os.path.join(current_path+"/logs", args.model)
